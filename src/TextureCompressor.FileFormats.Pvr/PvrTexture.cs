...
public sealed class PvrTexture
{
    ...
    public PvrTexture(TextureFormat format, int width, int height, byte[] payload, IReadOnlyList<PvrMetadata> metadata)
        : this(format, [new TextureSubresource(0, 0, 0, width, height, payload)], 1, 1, metadata)
    {
    }
    ...
    public PvrTexture(TextureFormat format, IReadOnlyList<TextureMipLevel> mipLevels, IReadOnlyList<PvrMetadata> metadata)
        : this(format, ToSubresources(mipLevels), 1, 1, metadata)
    {
    }
    ...
    public PvrTexture(TextureFormat format, IReadOnlyList<TextureSubresource> subresources, int arrayLayerCount, int faceCount)
        : this(format, subresources, arrayLayerCount, faceCount, [])
    {
    }
    ...
    {
        var resources = ValidateSubresources(subresources, arrayLayerCount, faceCount);
    ...
    private static TextureSubresource[] ToSubresources(IReadOnlyList<TextureMipLevel> mipLevels)
    {
        ...
        int faceCount)
    {
        ...
        {
            throw new ArgumentException("PVR texture must contain at least one subresource.", nameof(subresources));
        }
        ...
        {
            var subresource = subresources[i] ?? throw new ArgumentException("PVR subresource cannot be null.", nameof(subresources));
        ...
        {
            throw new ArgumentException("PVR texture is missing mip level 0 for array layer 0 face 0.", nameof(subresources));
        }
        ...
        {
            throw new ArgumentException("PVR mip level count exceeds the full mip chain for the base dimensions.", nameof(subresources));
        }
        ...
        if (subresources.Count != expectedCount)
        {
            throw new ArgumentException(
                $"PVR texture has {subresources.Count} subresources, but {expectedCount} were expected for {arrayLayerCount} array layer(s), {faceCount} face(s), and {mipLevelCount} mip level(s).",
                nameof(subresources));
        }
        ...
        for (var i = 0; i < subresources.Count; i++)
        {
            ...
            {
                throw new ArgumentException(
                    $"PVR subresource array layer {subresource.ArrayLayer} is outside the declared array layer count {arrayLayerCount}.",
                    nameof(subresources));
            }
            ...
            {
                throw new ArgumentException(
                    $"PVR subresource face {subresource.FaceIndex} is outside the declared face count {faceCount}.",
                    nameof(subresources));
            }
            ...
            {
                throw new ArgumentException(
                    $"PVR mip level {subresource.MipLevel} is {subresource.Width}x{subresource.Height}, but {expectedWidth}x{expectedHeight} was expected.",
                    nameof(subresources));
            }
            ...
            {
                throw new ArgumentException(
                    $"PVR texture contains duplicate subresources for mip level {subresource.MipLevel}, array layer {subresource.ArrayLayer}, face {subresource.FaceIndex}.",
                    nameof(subresources));
            }
        ...
        for (var i = 0; i < resources.Length; i++)
        {
            ...
            {
                throw new ArgumentException("PVR texture is missing one or more subresources.", nameof(subresources));
            }
        }
