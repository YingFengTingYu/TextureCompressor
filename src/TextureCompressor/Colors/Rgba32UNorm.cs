using TextureCompressor.Formats;

namespace TextureCompressor.Colors;

public struct Rgba32UNorm(uint red, uint green, uint blue, uint alpha = uint.MaxValue)
    : IConvertibleToRgba8UNorm<Rgba32UNorm>,
      IConvertibleToRgba8SNorm<Rgba32UNorm>,
      IConvertibleToRgba16UNorm<Rgba32UNorm>,
      IConvertibleToRgba16SNorm<Rgba32UNorm>,
      IConvertibleToRgba32UNorm<Rgba32UNorm>,
      IConvertibleToRgba32SNorm<Rgba32UNorm>,
      IConvertibleToRgba16Float<Rgba32UNorm>,
      IConvertibleToRgba32Float<Rgba32UNorm>
{
    public uint Red = red;
    public uint Green = green;
    public uint Blue = blue;
    public uint Alpha = alpha;

    public static TextureFormat Format => TextureFormats.Rgba32UNorm;

    public static Rgba8UNorm ToRgba8UNorm(Rgba32UNorm value)
    {
        return new Rgba8UNorm(
            RgbaColorConversions.ToUNorm8(value.Red),
            RgbaColorConversions.ToUNorm8(value.Green),
            RgbaColorConversions.ToUNorm8(value.Blue),
            RgbaColorConversions.ToUNorm8(value.Alpha));
    }

    public static Rgba32UNorm FromRgba8UNorm(Rgba8UNorm value)
    {
        return new Rgba32UNorm(
            RgbaColorConversions.ToUNorm32(value.Red),
            RgbaColorConversions.ToUNorm32(value.Green),
            RgbaColorConversions.ToUNorm32(value.Blue),
            RgbaColorConversions.ToUNorm32(value.Alpha));
    }

    public static Rgba8SNorm ToRgba8SNorm(Rgba32UNorm value)
    {
        return new Rgba8SNorm(
            RgbaColorConversions.ToSNorm8(value.Red),
            RgbaColorConversions.ToSNorm8(value.Green),
            RgbaColorConversions.ToSNorm8(value.Blue),
            RgbaColorConversions.ToSNorm8(value.Alpha));
    }

    public static Rgba32UNorm FromRgba8SNorm(Rgba8SNorm value)
    {
        return new Rgba32UNorm(
            RgbaColorConversions.ToUNorm32(value.Red),
            RgbaColorConversions.ToUNorm32(value.Green),
            RgbaColorConversions.ToUNorm32(value.Blue),
            RgbaColorConversions.ToUNorm32(value.Alpha));
    }

    public static Rgba16UNorm ToRgba16UNorm(Rgba32UNorm value)
    {
        return new Rgba16UNorm(
            RgbaColorConversions.ToUNorm16(value.Red),
            RgbaColorConversions.ToUNorm16(value.Green),
            RgbaColorConversions.ToUNorm16(value.Blue),
            RgbaColorConversions.ToUNorm16(value.Alpha));
    }

    public static Rgba32UNorm FromRgba16UNorm(Rgba16UNorm value)
    {
        return new Rgba32UNorm(
            RgbaColorConversions.ToUNorm32(value.Red),
            RgbaColorConversions.ToUNorm32(value.Green),
            RgbaColorConversions.ToUNorm32(value.Blue),
            RgbaColorConversions.ToUNorm32(value.Alpha));
    }

    public static Rgba16SNorm ToRgba16SNorm(Rgba32UNorm value)
    {
        return new Rgba16SNorm(
            RgbaColorConversions.ToSNorm16(value.Red),
            RgbaColorConversions.ToSNorm16(value.Green),
            RgbaColorConversions.ToSNorm16(value.Blue),
            RgbaColorConversions.ToSNorm16(value.Alpha));
    }

    public static Rgba32UNorm FromRgba16SNorm(Rgba16SNorm value)
    {
        return new Rgba32UNorm(
            RgbaColorConversions.ToUNorm32(value.Red),
            RgbaColorConversions.ToUNorm32(value.Green),
            RgbaColorConversions.ToUNorm32(value.Blue),
            RgbaColorConversions.ToUNorm32(value.Alpha));
    }

    public static Rgba32UNorm ToRgba32UNorm(Rgba32UNorm value)
    {
        return value;
    }

    public static Rgba32UNorm FromRgba32UNorm(Rgba32UNorm value)
    {
        return value;
    }

    public static Rgba32SNorm ToRgba32SNorm(Rgba32UNorm value)
    {
        return new Rgba32SNorm(
            RgbaColorConversions.ToSNorm32(value.Red),
            RgbaColorConversions.ToSNorm32(value.Green),
            RgbaColorConversions.ToSNorm32(value.Blue),
            RgbaColorConversions.ToSNorm32(value.Alpha));
    }

    public static Rgba32UNorm FromRgba32SNorm(Rgba32SNorm value)
    {
        return new Rgba32UNorm(
            RgbaColorConversions.ToUNorm32(value.Red),
            RgbaColorConversions.ToUNorm32(value.Green),
            RgbaColorConversions.ToUNorm32(value.Blue),
            RgbaColorConversions.ToUNorm32(value.Alpha));
    }

    public static Rgba16Float ToRgba16Float(Rgba32UNorm value)
    {
        return new Rgba16Float(
            RgbaColorConversions.ToHalf(value.Red),
            RgbaColorConversions.ToHalf(value.Green),
            RgbaColorConversions.ToHalf(value.Blue),
            RgbaColorConversions.ToHalf(value.Alpha));
    }

    public static Rgba32UNorm FromRgba16Float(Rgba16Float value)
    {
        return new Rgba32UNorm(
            RgbaColorConversions.ToUNorm32(value.Red),
            RgbaColorConversions.ToUNorm32(value.Green),
            RgbaColorConversions.ToUNorm32(value.Blue),
            RgbaColorConversions.ToUNorm32(value.Alpha));
    }

    public static Rgba32Float ToRgba32Float(Rgba32UNorm value)
    {
        return new Rgba32Float(
            RgbaColorConversions.ToFloat(value.Red),
            RgbaColorConversions.ToFloat(value.Green),
            RgbaColorConversions.ToFloat(value.Blue),
            RgbaColorConversions.ToFloat(value.Alpha));
    }

    public static Rgba32UNorm FromRgba32Float(Rgba32Float value)
    {
        return new Rgba32UNorm(
            RgbaColorConversions.FloatToUNorm32(value.Red),
            RgbaColorConversions.FloatToUNorm32(value.Green),
            RgbaColorConversions.FloatToUNorm32(value.Blue),
            RgbaColorConversions.FloatToUNorm32(value.Alpha));
    }
}
