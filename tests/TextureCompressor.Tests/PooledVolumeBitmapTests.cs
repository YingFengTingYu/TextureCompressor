...
public sealed class PooledVolumeBitmapTests
{
    ...
    public void ArrayPoolConstructorRentsAndReturnsBuffer()
    {
        ...
        Assert.Equal(8, pool.RentedMinimumLength);
    ...
    public void MemoryPoolConstructorDisposesOwner()
    {
        ...
        bitmap.PixelSpan[5] = new Rgba8UNorm(30, 31, 32);

        Assert.Equal(8, pool.RentedMinimumLength);
        Assert.Equal(new Rgba8UNorm(30, 31, 32), pool.Owner!.Buffer[5]);
